#!/usr/bin/env node
// T2.1 spike 引导脚本：确保 X11 虚拟显示（Xvfb）与 DevTools IDE（服务端口）就绪。
//
// 复用 scripts/devtools-build.js 的「Xvfb :97 + IDE 冷启动 + 服务端口」逻辑：
//   1) 无 X11 显示时在本机起 Xvfb（:97，可用 WDT_DISPLAY 覆盖）；
//   2) IDE 未运行时拉起（--disable-gpu --enable-service-port）并等待端口文件；
//   3) 打印解析出的 CLI 路径（e2e/spike/run.js 用同样逻辑取 cliPath）。
//
// automator.launch() 内部通过 `cli auto --project <path> --auto-port <port>`
// 驱动 IDE；本脚本只负责它的前置条件（显示 + IDE 服务端口），不代替 automator
// 打开工程。
'use strict';
...
function main() {
  if (!fs.existsSync(CLI)) {
    console.error(`[spike-ensure] ❌ 未找到 CLI：${CLI}\n   用 scripts/install-devtools.sh 安装，或设 DEVTOOLS_CLI/DEVTOOLS_ROOT。`);
    process.exit(2);
  }
  if (!fs.existsSync(NW_BIN)) {
    console.error(`[spike-ensure] ❌ 未找到 nwjs：${NW_BIN}`);
    process.exit(2);
  }
  // 每轮干净重启 IDE，避免残留 auto tunnel 干扰 automator 端口探测。
  stopAllIde();
