...
//     global.setup.js    -> await startRecording()（在 ensureDevtools 之前，
//                           录像覆盖 Xvfb 启动、IDE 冷启、打开工程编译、全程测试）
//     global.teardown.js -> await stopRecording()（在 stopAllIde 之前，
//                           避免录像以进程被杀的黑屏收尾）
...
//
// 登录/授权浮层：不再用 drawbox 遮罩掩盖（owner 已拍板「摘掉各种遮罩」）。登录态由
// e2e/tools/login-stub.js 在冷启前注入（root 修复），未登录浮层不出现，录像即干净。
// 若未来 IDE 漂移导致登录浮层再现，应修 login-stub 而非加遮罩。
//
// 环境变量：
//   E2E_RECORD       =1 开启（默认关）
